...
h=ones(1,length(t));
fc=1;
itrain=(square(2*pi*20*f c*t)+1)/2;
X = sin(2*pi*fc*t);
Y=X.*itrain;
figure(1);
subplot(3,1,1);
...
%{
%Frequency Spectrum of X_demod
figure(8)
fft_demod=fft(X_demod);
len=length(X_demod);
w=[-len/2:len/2-1];
plot(w,abs(fft(X_demod)))
...
input_fft=fft(X_demod);   		 	%fft of input signal
fs=100;
wn=2/(fs/2); 					%lowpass
[b,a]=butter(2,wn);  				%Choosing order as random N=2 here
f=filter(b,a,X_demod); 				%Filter design
output_fft=fft(f);

%{
figure;
plot(real(output_fft));
...
figure;
y=smooth(X_demod,0.1);
plot(y,'linewidth',1.5);
title('\color{red} RECONSTRUCTION FILTER USING SMOOTH FUNCTION');
xlabel('frequency');
ylabel('Amplitude');
